...
console.log(name);
    ...
    res = db.query("select * from Users where userName LIKE ?",[name],callback);
  ...
  addNewUser:function(newUser,callback){
    ...
    db.query("select max(usersID) AS maxId from Users",function (err,rows) {
      ...
      if(!err) {
        ...
        db.query("Insert into Users values(?,?,?,?,?,?,?,?)",[maxId,newUser.userName,newUser.password,newUser.email,newUser.fName,newUser.lName,
          newUser.phone,newUser.uType],callback);
      }
    });
  },
  ...
  updateUsersById: function(id,user,callback){
    ...
    db.connect(function (err) {
      ...
      res = db.query("update Users set username=? , password=? , email = ?, fName = ? , lName = ?,phone = ? , utype=?  " +
        " where usersID=?",[user.userName, user.password, user.email, user.fName, user.lName, user.phone, user.uType,id],callback);
